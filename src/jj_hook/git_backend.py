...
class GitBackend(VCSBackend):
        ...
        if success:
            ...
            if message:
                commit_success, commit_stdout, commit_stderr = self.run_command(
                    ["git", "commit", "--allow-empty", "-m", message], timeout=15
                )
                if commit_success:
    ...
    def get_commit_log(self, limit: int = 20) -> Tuple[bool, str]:
        ...
        success, stdout, stderr = self.run_command(
            ["git", "log", "--oneline", f"-{limit}"], timeout=15
        )
        if success:
            return True, stdout.strip()
        else:
    ...
    def get_commit_diff_stat(self, commit_hash: str) -> Tuple[bool, str]:
        """指定されたコミットの差分統計を取得する。"""
        success, stdout, stderr = self.run_command(
            ["git", "show", "--stat", "--format=", commit_hash], timeout=10
        )
        if success:
            return True, stdout.strip()
        else:
    ...
    def get_commit_message(self, commit_hash: str) -> Tuple[bool, str]:
        ...
        success, stdout, stderr = self.run_command(
            ["git", "log", "-1", "--pretty=format:%s", commit_hash], timeout=10
        )
        if success:
            return True, stdout.strip()
        else:
    ...
    def get_changed_files(self, commit_hash: str) -> Tuple[bool, list[str]]:
        ...
        success, stdout, stderr = self.run_command(
            ["git", "show", "--name-only", "--format=", commit_hash], timeout=10
        )
        if success:
    ...
    def create_backup_branch(self, name: str) -> Tuple[bool, str]:
        ...
        success, stdout, stderr = self.run_command(["git", "branch", name], timeout=10)
        if success:
            return True, name
        else:
            return False, stderr.strip()
    ...
    def interactive_rebase(self, base_commit: str) -> Tuple[bool, str]:
        ...
        # 自動化環境では使用注意
        success, stdout, stderr = self.run_command(["git", "rebase", "-i", base_commit], timeout=60)
        if success:
    ...
    def squash_commits(self, commit_hashes: list[str], new_message: str) -> Tuple[bool, str]:
        ...
        success, parent_stdout, stderr = self.run_command(
            ["git", "rev-parse", f"{oldest_commit}^"], timeout=10
        )
        ...
        success, _, stderr = self.run_command(["git", "reset", "--soft", parent_hash], timeout=15)

        if not success:
            return False, f"リセットに失敗: {stderr}"

        # 新しいメッセージでコミット
        success, stdout, stderr = self.run_command(["git", "commit", "-m", new_message], timeout=30)
