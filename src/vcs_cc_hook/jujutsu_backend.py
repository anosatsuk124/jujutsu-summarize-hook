...
class JujutsuBackend(VCSBackend):
    ...
    def get_commit_diff_stat(self, commit_id: str) -> Tuple[bool, str]:
        """指定されたコミットの差分統計を取得する。"""
        success, stdout, stderr = self.run_command(
            ["jj", "diff", "-r", commit_id, "--stat"], timeout=10
        )
        if success:
            return True, stdout.strip()
        else:
    ...
    def get_commit_message(self, commit_id: str) -> Tuple[bool, str]:
        ...
        success, stdout, stderr = self.run_command(
            ["jj", "log", "-r", commit_id, "-T", "description"], timeout=10
        )
        if success:
            return True, stdout.strip()
        else:
    ...
    def get_changed_files(self, commit_id: str) -> Tuple[bool, list[str]]:
        ...
        success, stdout, stderr = self.run_command(
            ["jj", "diff", "-r", commit_id, "--name-only"], timeout=10
        )
        if success:
    ...
    def create_backup_bookmark(self, name: str) -> Tuple[bool, str]:
        """バックアップブックマークを作成する。"""
        success, stdout, stderr = self.run_command(
            ["jj", "bookmark", "create", name, "-r", "@"], timeout=10
        )
        if success:
            return True, name
        else:
    ...
    def squash_commits(
        self, source_commit: str, target_commit: str, new_message: Optional[str] = None
    ) -> Tuple[bool, str]:
        """コミットをスカッシュする（jj squash）。"""
        success, stdout, stderr = self.run_command(
            ["jj", "squash", "--from", source_commit, "--into", target_commit], timeout=30
        )
        ...
        if new_message:
            msg_success, msg_stdout, msg_stderr = self.run_command(
                ["jj", "describe", "-r", target_commit, "-m", new_message], timeout=15
            )
    ...
    def update_commit_message(self, commit_id: str, message: str) -> Tuple[bool, str]:
        """コミットメッセージを更新する。"""
        success, stdout, stderr = self.run_command(
            ["jj", "describe", "-r", commit_id, "-m", message], timeout=15
        )
        if success:
            return True, stdout.strip()
        else:
    ...
    def create_bookmark(self, name: str, revision: str = "@") -> Tuple[bool, str]:
        """ブックマークを作成する。"""
        success, stdout, stderr = self.run_command(
            ["jj", "bookmark", "create", name, "-r", revision], timeout=10
        )
        if success:
    ...
    def get_current_revision(self) -> Tuple[bool, str]:
        """現在のリビジョンIDを取得する。"""
        success, stdout, stderr = self.run_command(
            ["jj", "log", "-r", "@", "-T", "commit_id"], timeout=5
        )
        if success:
            return True, stdout.strip()
        else:
