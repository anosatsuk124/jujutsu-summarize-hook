...
def check_github_copilot_auth() -> tuple[bool, str]:
    ...
    try:
        ...
        # Git専用環境変数を優先チェック
        model_env = (
            os.environ.get("GIT_CC_HOOK_MODEL")
...
def authenticate_github_copilot() -> bool:
    ...
    try:
        ...
        if not config.model.startswith("github_copilot/"):
            console.print(
                "[yellow]警告: GIT_CC_HOOK_MODELがGitHub Copilotモデルに設定されていません[/yellow]"
            )
...
def get_slash_command_content(template_name: str = "slash_command") -> str:
    ...
    template_loader = TemplateLoader(vcs_type="git")
...
このサブエージェントはGitリポジトリのコミット履歴を解析し、整理の提案を行います。
"""
...
def auth(provider: str, check: bool) -> None:
    """LLMプロバイダーの認証を行う（Git専用）。

    PROVIDER: 認証するプロバイダー (github-copilot)

    例:
    \b
    git-cc-hook auth github-copilot    # GitHub Copilot認証を実行
    git-cc-hook auth --check           # 認証状態を確認
    """
    if check:
    ...
    if provider == "github-copilot":
        ...
        if success:
            ...
            console.print("[dim]これで git-cc-hook を使用する準備が整いました。[/dim]")
        else:
...
def install_agent(is_global: bool, dry_run: bool, path: Optional[Path]) -> None:
    """Git専用のサブエージェント (git-commit-organizer) をClaude Codeに追加する。"""
    ...
    agent_file = agents_dir / "git-commit-organizer.md"
    ...
    try:
        ...
        try:
            ...
            # フォールバック内容
            agent_content = """# git-commit-organizer

Gitリポジトリのコミット履歴を解析し、コミットの整理を提案するサブエージェントです。
...
```
git-cc-hook analyze --interactive
```

または、直接呼び出し:

```
Task(description="git-commit-organizer でコミット履歴を整理", prompt="Gitリポジトリのコミット履歴を解析し、整理の提案をお願いします", subagent_type="git-commit-organizer")
```
"""
        ...
        console.print(
            Panel(
                Text(
                    "git-commit-organizer サブエージェントのインストールが完了しました！\n\n"
                    "使用方法:\n"
                    "• Claude Code内で 'git-commit-organizer' と呼び出し\n"
                    '• Task(subagent_type="git-commit-organizer") でプログラム的に実行\n'
                    "• /git-commit-organizer スラッシュコマンド（別途インストール要）\n\n"
...
def install_slash_command(is_global: bool, dry_run: bool, path: Optional[Path]) -> None:
    """Git専用のスラッシュコマンド (/git-commit-organizer) をClaude Codeに追加する。"""
    ...
    try:
        ...
        console.print(
            Panel(
                Text(
                    "/git-commit-organizer スラッシュコマンドのインストールが完了しました！\n\n"
                    "使用方法:\n"
                    "• Claude Code内で '/git-commit-organizer' と入力\n"
                    "• 自動的にGitコミット整理機能が実行されます\n\n"
...
def install_all(is_global: bool, dry_run: bool, path: Optional[Path]) -> None:
    """Git専用のすべてのコンポーネント（フック、サブエージェント、スラッシュコマンド）を一括インストールする。"""
    ...
    console.print(f"[blue]🚀 git-cc-hook 一括インストールを開始します[/blue]")
    console.print(f"[blue]インストール先: {install_location}[/blue]")
    ...
    try:
        ...
        # 2. サブエージェントのインストール
        console.print("\n[cyan]2. サブエージェント (git-commit-organizer) のインストール...[/cyan]")
...
"""
        ...
        # 3. スラッシュコマンドのインストール
        console.print(
            "\n[cyan]3. スラッシュコマンド (/git-commit-organizer) のインストール...[/cyan]"
        )
        ...
        console.print(
            Panel(
                Text(
                    "🎉 git-cc-hook (Git専用) の一括インストールが完了しました！\n\n"
                    "インストールされたコンポーネント:\n"
                    "✅ Claude Code フック (PreToolUse, PostToolUse)\n"
                    "✅ git-commit-organizer サブエージェント\n"
                    "✅ /git-commit-organizer スラッシュコマンド\n\n"
                    "使用方法:\n"
                    "• ファイル編集で自動フック実行\n"
                    "• Claude Code内で 'git-commit-organizer' と呼び出し\n"
                    "• '/git-commit-organizer' スラッシュコマンド実行\n"
                    "• git-cc-hook auth github-copilot (認証設定)\n\n"
